...
def is_white_dominant(roi, threshold=0.2):
    # Convert to grayscale or operate directly on BGR
    # Assume in BGR image, white pixels have all three channels close to 255
    white_mask = np.all(roi > 200, axis=2)  # All channels > 240 are considered white
    white_ratio = np.sum(white_mask) / (roi.shape[0] * roi.shape[1])
    log(f"White pixel ratio: {white_ratio:.2f} (threshold: {threshold})")
    return white_ratio > threshold

def find_max_red_region(img, search_rect, box_size=7, threshold=RED_THRESHOLD):
    """
    Use sliding window in specified large region to find small region with highest red pixel ratio
    Returns (x1, y1, x2, y2), max_ratio
    """
...
def is_blue_target(img, roi, color_list, tolerance=8):
    ...
    for color in color_list:
        ...
        if np.all(diff <= tolerance):
            print(f"Detected target cyan color: {color}, actual={avg_color[::-1]}")
            return True
    return False
